...
<div class="container">
	...
	$res=mysqli_query($db,"SELECT * FROM `fine`;");
		...
		<?php
		if(isset($_POST['srch'])){
		$total_fine = mysqli_query($db,"SELECT SUM(fine) AS total_fine FROM `fine` 
		where username like '%$_POST[search]%' ");
		$row = mysqli_fetch_assoc($total_fine);
		echo "<tr>";
		echo "<td colspan='4' align='right'><b>Total Fine: </b></td>";
		echo "<td><b>$ ".$row['total_fine']."</b></td>";
		echo "<td></td>";
		echo "</tr>";
		echo "</table>";}?>
